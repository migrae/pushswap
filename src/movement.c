...
t_stacks	*lowest_a_to_b(t_stacks *stacks)
{
	...
	if (middle >= get_stack_pos(lowest, stacks->a))
	{
		...
		while (get_stack_pos(lowest, stacks->a) != 1)
			stacks->a = rev_rotation(stacks->a, rotate_a);
	send_num_to_stack(&stacks->a, &stacks->b, push_a);
	return (stacks);
}
...
t_stacks	*highest_to_top_b(t_stacks *stacks)
{
	...
	if (middle > get_stack_pos(highest, stacks->b))
	{
		...
		while (get_stack_pos(highest, stacks->b) != 1)
			stacks->b = rev_rotation(stacks->b, rotate_b);
	return (stacks);
}
...
t_stacks	*chunk_to_b(t_stacks *stacks)
{
	...
	while (i < stacks->chunk_size && temp)
	{
		...
		{
			stacks = one_chunknum_to_b(stacks, temp);
			i++;
			temp = stacks->a;
		}
		else
